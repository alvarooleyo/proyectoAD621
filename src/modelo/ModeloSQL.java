...
public class ModeloSQL extends DatabaseSQL{
    ...
    public DefaultTableModel getTablaRecaudaciones() {
        ...
        try {
            //llamamos a la funcion almacenada en la base de datos
            CallableStatement pstm = this.getConexion().prepareCall("{?=call totalRecaudacion}");
            //detallamos que la variable de salida va a ser de tipo Integer
            pstm.registerOutParameter(1, Types.INTEGER);
            //ejecutamos la llamada a la funcion
            pstm.executeUpdate();
        ...
        try {
            //Preparas la llamada al procedimiento que te devuelve la consulta
            CallableStatement cstm = this.getConexion().prepareCall("{call consultaRecaudacion()}");
            //Ejecutas el procedimiento
            ResultSet re = cstm.executeQuery();
            //Recoges los metadatos que te devuelve la consulta
            ResultSetMetaData rsp = re.getMetaData();
    ...
    public int modificarProducto2(int codProd, String nombreProd, int cantidadProd, double precioProd){
        int aux=0;
        try {
            CallableStatement cStmt= this.getConexion().prepareCall("{?= call updateProducto(?,?,?,?)}");
            cStmt.registerOutParameter(1, java.sql.Types.INTEGER);
            cStmt.setInt(2, codProd);
            cStmt.setString(3,nombreProd);
            cStmt.setInt(4, cantidadProd);
            cStmt.setDouble(5, precioProd);
    ...
    public int eliminarProducto(int codProd){
        int aux=0;
        try {
            CallableStatement cStmt= this.getConexion().prepareCall("{?= call deleteProducto(?)}");
            cStmt.registerOutParameter(1, java.sql.Types.INTEGER);
            cStmt.setInt(2,codProd);
            cStmt.execute();
            aux= cStmt.getInt(1);
    ...
    public void generarInformeEmpleados(String CIF) {
    //Escoge la ruta en el que se encuentra el documento
        String path = this.getClass().getProtectionDomain().getCodeSource().getLocation().getPath();
